            ...
            <div class="profile__user user container">
                ...
                <div class="profile__rating user__rating">
                    <p class="profile__rating-item user__rating-item user__rating-item--publications">
                        <span class="user__rating-amount"><?= $userProfileData['count_posts'] ?? '0' ?></span>
                        <span class="profile__rating-text user__rating-text">
                            <?= isset($userProfileData['count_posts']) ? showAuthorPostsCount($userProfileData['count_posts']) : 'постов' ?>
                        </span>
                    </p>
                    <p class="profile__rating-item user__rating-item user__rating-item--subscribers">
                        <span class="user__rating-amount"><?= $userProfileData['subscribers'] ?: '0' ?></span>
                        <span class="profile__rating-text user__rating-text">
                            <?= isset($userProfileData['subscribers']) ? showSubscribersCount($userProfileData['subscribers']) : 'подписчиков' ?>
                        </span>
                    </p>
                </div>
                <div class="profile__user-buttons user__buttons">
                    <?php
                    if (isset($userProfileData['id']) && $userProfileData['id'] !== $authorizedUser): ?>
                        <?php
                        $button = 'button--main';
