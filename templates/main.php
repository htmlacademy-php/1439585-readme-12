    ...
    <div class="popular container">
        ...
        <?php if (!empty($cards)): ?>
            ...
            <?php foreach ($cards as $card): ?>
                <?php foreach ($categories as $category) {
                      if (isset($card['category_id']) && isset($category['id']) && (int)$card['category_id'] === (int)$category['id']) {
                    ...
                    <div class="post__main">
                        ...
                        <?php elseif ($postType === 'post-link'): ?>
                            <div class="post-link__wrapper">
                                <a class="post-link__external" href="<?= isset($card['website_link']) ? correctSiteUrl(htmlspecialchars($card['website_link'])) : '' ?>" title="Перейти по ссылке">
                        ...
                        <?php elseif ($postType === 'post-video'): ?>
                            <div class="post-video__block">
                                <div class="post-video__preview">
                                    <?= isset($card['video_link']) ? embed_youtube_cover($card['video_link']) : '' ?>
                                </div>
                        ...
                        <?php elseif ($postType === 'post-text'): ?>
                            <p>
                                <?= (isset($card['content']) && isset($card['post_id'])) ? cutCardContent($card['content'], $card['post_id']) : '' ?>
                            </p>
                        <?php endif; ?>
                    </div>
                        ...
                        <div class="post__author">
                            <a class="post__author-link" href="profile.php?profile_id=<?= $card['user_id'] ?? '' ?>" title="Автор">
                                ...
                                <div class="post__info">
                                    ...
                                    <?php $postDate = isset($card['date_add']) ? showDate($card['date_add']) : ''; ?>
        ...
        <?php else: ?>
            <p><?= 'Здесь пока ничего нет. Но вы можете быть первыми! =)' ?></p>
            <p><?= 'Просто добавьте пост!' ?></p>
        <?php endif; ?>
        <?php if ($countPosts > 6 ): ?>
